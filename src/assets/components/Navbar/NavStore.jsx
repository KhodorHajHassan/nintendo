  ...
  return (
    <div
      className={`bg-white fixed flex justify-center items-center p-8 gap-8  z-40 left-0 w-full duration-100 ${
        selected ? "h-40 top-24 border-t-[1px]" : "h-0 -top-24"
      }`}
    >
       <div onClick={()=>setShow(!show)} className="absolute top-1 right-4 l text-gray-400  text-3xl cursor-pointer">
       <IoIosCloseCircle />
        </div> 
      {selected &&
        selected.map((item, index) => (
          <div className="flex flex-col gap-2 " key={index}>
            <div className="border-[1px] py-2 px-8 rounded-[1rem] hover:bg-secondary duration-300 cursor-pointer border-gray-300 flex justify-center items-center">
